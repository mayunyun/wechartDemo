...
@interface XZJActionShitPicker()
@property (nonatomic,strong) NSArray *data;
@property (nonatomic,assign) NSInteger selectedIndex;
@end

@implementation XZJActionShitPicker

+ (instancetype)showPickerWithTitle:(NSString *)title rows:(NSArray *)strings initialSelection:(NSInteger)index doneBlock:(XZJActionStringDoneBlock)doneBlock cancelBlock:(XZJActionStringCancelBlock)cancelBlockOrNil origin:(id)origin {
    XZJActionShitPicker * picker = [[XZJActionShitPicker alloc] initWithTitle:title rows:strings initialSelection:index doneBlock:doneBlock cancelBlock:cancelBlockOrNil origin:origin];
...
+ (instancetype)showPickerWithTitle:(NSString *)title rows:(NSArray *)data initialSelection:(NSInteger)index target:(id)target successAction:(SEL)successAction cancelAction:(SEL)cancelActionOrNil origin:(id)origin {
    XZJActionShitPicker *picker = [[XZJActionShitPicker alloc] initWithTitle:title rows:data initialSelection:index target:target successAction:successAction cancelAction:cancelActionOrNil origin:origin];
    [picker showActionSheetPicker];
    return picker;
}
...
- (UIView *)configuredPickerView {
    ...
    stringPicker.backgroundColor = ColorWithRGB(210, 212, 219, 1);
