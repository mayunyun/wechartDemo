...
static UIImage  *_yeeRedDotImage(CGFloat redDotRadius,UIColor  *redDotColor , CGSize lableSize){
    ...
    UIGraphicsBeginImageContextWithOptions(CGSizeMake(lableSize.width+11,lableSize.height),NO, [UIScreen mainScreen].scale);
    UIBezierPath* roundedRectanglePath = [UIBezierPath bezierPathWithRoundedRect:CGRectMake(0, 0, lableSize.width+11, lableSize.height) cornerRadius:redDotRadius];
    [redDotColor setFill];
    [roundedRectanglePath fill];
    UIImage *image= UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
...
- (void)initConfigureView{
    ...
    _redDotNumber    =0;
    _redDotMaxNumber =99;
    _redDotColor     =ColorWithRGB(250, 81, 80, 1);//[UIColor redColor];
...
- (void)setRedDotNumber:(NSInteger)redDotNumber{
    ...
    self.m_pBadgeLable.text =[NSString stringWithFormat:@"%@",redDotNumber>_redDotMaxNumber?@"...":@(redDotNumber)];
    [self update_subViews];
}
...
- (UILabel*)m_pBadgeLable{
    ...
    if (!_m_pBadgeLable) {
        ...
        _m_pBadgeLable.font=[UIFont systemFontOfSize:15];
